...
### Slack プロキシ デバッグサーバー追加

- [x] **T12** `tools/slack_proxy_debug.py` 新規作成
  - 標準ライブラリのみ（`http.server`）で実装
  - `POST /post` でリクエストボディを JSON パースし、標準出力に整形して印字
  - `{"ok": true}` を JSON レスポンスとして返す
  - ポート: 8081

- [x] **T13** `Dockerfile.slack_proxy_debug` 新規作成
  - `python:3.12-slim` ベース
  - `tools/slack_proxy_debug.py` のみをコピーして起動
  - 標準ライブラリのみのため `uv` / 依存インストール不要

- [x] **T14** `docker-compose.yml` 更新
  - `slack_proxy_debug` サービスを追加（ポート `8081:8081`、`chatops_net` 参加）
  - `api` サービスに `SLACK_PROXY_URL: http://slack_proxy_debug:8081/post` を追記
  - `worker` サービスに `SLACK_PROXY_URL: http://slack_proxy_debug:8081/post` を追記
