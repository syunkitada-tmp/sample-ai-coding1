...
├── docker-compose.yml                # db / migrate / api / worker / slack_proxy_debug
├── Dockerfile.api
├── Dockerfile.worker
├── Dockerfile.slack_proxy_debug      # デバッグサーバー用（python:3.12-slim、標準ライブラリのみ）
└── pyproject.toml
```
...
必須フィールドは `command_name: str`、`description: str`、`execute(kwargs, args, thread_context) -> str` の3つ。
...
class BasePlugin(ABC):
    ...
    @abstractmethod
    def execute(self, kwargs: dict[str, str | bool], args: str, thread_context: dict) -> str:
        ...
```
