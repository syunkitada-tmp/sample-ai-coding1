...
2. 開発者に提案内容を提示し、明確な指摘がなければ承認を待たずに作成・切り替えを行う（`git checkout -b <ブランチ名>`）
...
Conventional Commits 形式でメッセージを生成し、開発者に提示してください（明確な指摘がなければ承認を待たずにコミットすること）。
...
コミットメッセージに明確な指摘がなければ、承認を待たずに以下を実行してください：

1. `git add` で対象ファイルをステージング（Step 2 で確定した単位で）
2. `git commit` でコミット（Step 3 で生成したメッセージで）
...
> **コミット完了！** 変更内容をコミットしました。
> ※ ブランチ名やコミットメッセージに問題がある場合は、`git commit --amend` 等で修正しますのでお知らせください。
