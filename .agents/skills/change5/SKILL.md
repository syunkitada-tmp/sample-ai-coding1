...
それ以外の場合は、実装方針を提示してください（明確な指摘がなければ承認を待たずに実装に進むこと）。実装前に方針を明確にすることが重要です。

### Step 3: TDD サイクルで実装

方針について明確な指摘がなければ、承認を待たずに TDD サイクル（Red-Green-Refactor）で実装を進めてください：

1. **Red**: テストを書き、失敗することを確認
2. **Green**: テストをパスする最小限のコードを書く
3. **Refactor**: コードを整理する（振る舞いを変えずに可読性・保守性を向上）
...
- タスク完了と次のアクションが開発者に提示されていること（明示的な承認は待たずに次へ進む案内を出すこと）
...
> **タスク完了！** [タスク名] を実装しました。
> ※ 実装内容に問題や修正したい点があれば、次へ進む前にこの場でお知らせください。
>
> 問題がなければ、引き続き次のタスクに進む場合は再度 `@skill change5` を実行してください。
> 全タスクが完了した場合は **Phase 6: 変更レビュー & ドキュメント更新** に進みます。`@skill change6` を実行してください。
